...
Operation.prototype.getInlineModel = function(inline_str) {
  if(/^Inline Model \d+$/.test(inline_str)) {
    var id = parseInt(inline_str.substr('Inline Model'.length).trim(),10); //
...
Operation.prototype.urlify = function (args) {
  ...
  if (args.queryParams || args.extraQueryParams) {
    var extra = args.queryParams || args.extraQueryParams;
    var keys = Object.keys(extra);
    for (var i = 0; i < keys.length; i++) {
      if (extra[keys[i]] !== undefined) {
        if (querystring === '') {
          querystring += '?';
        } else {
          querystring += '&';
        }
        querystring += this.encodeQueryParam(keys[i]) + '=' + this.encodeQueryParam(extra[keys[i]]);
      }
    }
  }
...
Operation.prototype.getModelSampleJSON = function (type, models) {
  var isPrimitive, listType, sampleJson, innerType;
  models = models || {};
